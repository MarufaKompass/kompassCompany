...
export default function AppointmentModal(props) {
  ...
  const { meetingModal, handleClose, meetingDetails } = props;

  useEffect(() => {
    const fetchData = () => {
      if (meetingDetails) {
        axiosInstance
          .get(`https://api.hellokompass.com/meeting/${meetingDetails}`)
          .then((res) => {
  ...
  return (
    <Box>
      <Modal open={Boolean(meetingModal)} onClose={handleClose} sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
        <Slide in={Boolean(meetingModal)}>
          <Box
            sx={{
              position: 'absolute',
