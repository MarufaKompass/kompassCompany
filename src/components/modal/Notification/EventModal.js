...
export default function EventModal(props) {
  ...
  const { invitation, openEventModal, handleClose } = props;
  ...
  const handleCloseModal = () => {
    handleClose();
    if (data?.is_read === 'no') {
      const invitationData = { id: data?.id, is_read: 'yes' };
      axiosInstance.post('https://api.hellokompass.com/notification/update', invitationData).then((res) => {
        console.log(res);
      });
    } else {
    }
  ...
  useEffect(() => {
    const fetchData = () => {
      if (data) {
        axiosInstance
          .get(`https://api.hellokompass.com/event/viewinvt/${data?.module_id}`)
          .then((res) => {
              ...
              <Box display="flex" justifyContent="center">
                <Box>
                  <Typography color="#12A9B2">
                    Status:
                    {status ? <CustomChip>{status}</CustomChip> : null}
                  </Typography>
