...
export default function DeleteScheduleModal({ deleteScheduleId, deleteScheduleModal, handleClose }) {
  const deleteData = (id) => {
    axiosInstance
      .delete(`https://api.hellokompass.com/calender/schedule/delete/${id}`)
      .then((res) => {
        if (res.status === 200) {
  ...
  return (
    <Box>
      <Modal open={Boolean(deleteScheduleModal)} onClose={handleClose}>
        <Box sx={{ ...style, width: 320 }}>
          ...
          <Box sx={{ px: 1 }} display="flex" justifyContent="center">
            <Box>
              <Button
                onClick={() => deleteData(deleteScheduleId)}
                variant="contained"
                size="small"
