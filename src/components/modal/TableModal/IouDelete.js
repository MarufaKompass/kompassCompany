...
export default function IouDelete({ iouId, iouDeleteModal, handleClose }) {
  const { profile } = useAppContext();
  const { company_id, employee_id } = profile;

  const deleteData = (eventDelete) => {
    const data = { emp_id: employee_id, com_id: company_id, iou_id: eventDelete };
    axiosInstance
      .post('https://api.hellokompass.com/payroll/ioudelete', data)
      .then((res) => {
        if (res.status === 200) {
  ...
  return (
    <Box>
      <Modal open={Boolean(iouDeleteModal)} onClose={handleClose}>
